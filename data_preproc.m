% written by hao (ver_17.07.12),developed by muen ,use to for data without
% year ID
% rock3.hao@gmail.com
% qinlab.BNU
%adaped by zhao muen for his data
restoredefaultpath
clear
...
% Set Path
spm_dir     = '.../spm12';
script_dir  = '.../preproc_spm12';
preproc_dir = '.../Data_CBD_noFM';
database='.../WM.xlsx';
[~,sublist,~]=xlsread(database,'WM','H:H');
%subjlist    = fullfile(script_dir,'SubList_ANT1_YesExist_Hao2.txt');

% Set Basic Information
fmri_name   = {'NB'}; % if multi run, use: {'Run1';'Run2'}
tr          = 2;
t1_filter   = 'I';
func_filter = 'I';
datat_type  = 'nii';
...
if preproc ==1
    for i = 4:length(sublist)
        for run = 1:length(fmri_name)%1:length(fmri_name);
           % YearID = ['20',sublist{i,1}(1:2)];
            SubjDir = sublist{i};
            disp ([SubjDir,' Preprocess Started'])
            
            T1Dir     = fullfile(preproc_dir,SubjDir,'/mri/anatomy/');
            FuncDir   = fullfile(preproc_dir,SubjDir,'/fmri/',fmri_name{run,1},'/unnormalized/');
            FinalDir = fullfile(preproc_dir,SubjDir,'/fmri/',fmri_name{run,1},'/smoothed_spm12/');
...
subjlist=fullfile('/brain/iCAN_admin/home/zhaomuen/SPM/spm_scripts/preproc_spm12/WM.txt');
% fid = fopen(subjlist); subjlist = {}; cntlist = 1;
% while ~feof(fid)
%     linedata = textscan(fgetl(fid), '%s', 'Delimiter', '\t');
%     subjlist(cntlist,:) = linedata{1}; cntlist = cntlist + 1; %#ok<*SAGROW>
% end
% fclose(fid);
    ...
    for k = 1:length(fmri_name)
        ...
        MoveExclu_spm12_Hao1(mConfigName)
    end
